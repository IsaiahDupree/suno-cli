...
async function downloadOneClip(page, clipId, format, clipMetadata = {}) {
  ...
  if (await downloadFileBtn.count() > 0) {
    ...
    // Verify downloaded file (DL-003)
    const verifyResult = verifyAudioFile(savePath, format);
    if (!verifyResult.valid) {
      console.warn(`⚠️  File verification failed for ${path.basename(savePath)}`);
    ...
    return { status: 'ok', file: path.basename(savePath), sizeMB, verified: verifyResult.valid };
  }
  ...
  try {
    ...
    await page.keyboard.press('Escape');
    await page.waitForTimeout(500);
    return { status: 'ok', file: path.basename(savePath), sizeMB: (fileSize / 1024 / 1024).toFixed(1), verified: verifyResult.valid };
  } catch (e) {
    await page.keyboard.press('Escape');
    await page.waitForTimeout(500);
